...
def unpaired_random_crop(img_lqs, img_refs, if_fix, patch_size):
    ...
    #print("unpaired_random_crop###############img_lqs.shape:\n",img_lqs.shape,img_lqs[0].shape,img_lqs[1].shape)
    if not isinstance(img_lqs, list):
        img_lqs = [img_lqs]
    ...
    #print(">>>>>>>>>>>>>>>>>>img_lqs.shape:\n",img_lqs[0].shape)
    #print("^^^^^^^^^^^^^^^^^h_ref, w_ref",h_ref, w_ref)
    delta = 0 
    belta =0   
    if h_ref < patch_size or w_ref < patch_size:
        if  h_ref > w_ref:
          h_ref =int(h_ref/w_ref*256) + belta
          w_ref = 256 + delta
          dim=[ w_ref, h_ref]
          img_refs[0] = cv2.resize(img_refs[0], dim, interpolation = cv2.INTER_AREA)
          #print("*******^h_ref,w_ref:",h_ref,w_ref)
          #print("*********img_refs.shape:\n",img_refs[0].shape)
        else:
          w_ref = int(w_ref/h_ref*256) + belta
          h_ref =256 + delta
          dim=[w_ref, h_ref]
          img_refs[0] = cv2.resize(img_refs[0], dim, interpolation = cv2.INTER_AREA)
    ...
    if h_lq < patch_size or w_lq < patch_size:
        ...
        #                 f'({patch_size}, {patch_size}). ')  
        if  h_lq > w_lq:
          h_lq =int(h_lq/w_lq)*256 + belta
          w_lq = 256 + delta
          dim=[w_lq, h_lq]
          img_lqs[0] = cv2.resize(img_lqs[0], dim, interpolation = cv2.INTER_AREA)
          
          #print("^^^^^^h_lq,w_lq:",h_lq,w_lq)
          #print("^^^^^^img_lqs.shape:\n",img_lqs[0].shape)
        else:
          w_lq = int(w_lq/h_lq*256) + belta
          h_lq =256 + delta
          #print("!!!!!!!!!!!w_lq/h_lq:",w_lq/h_lq)
          dim=[w_lq, h_lq]
          img_lqs[0] = cv2.resize(img_lqs[0], dim, interpolation = cv2.INTER_AREA)
    ...
    #print(">!!!!!!!!!!!!!!1>img_lqs.shape:\n",img_lqs[0].shape)
    # randomly choose top and left coordinates for lq patch
    if if_fix:
        top = random.randint(0, h_lq - patch_size)
        #print("w_lq - patch_size",w_lq, patch_size,w_lq - patch_size)
        left = random.randint(0, w_lq - patch_size)
        img_lqs = [v[top:top + patch_size, left:left + patch_size, ...] for v in img_lqs]
